        ...
        try {
            ...
            if (c.moveToFirst()) {
                while (!c.isAfterLast()) {
                    ...
                    if(percentage>0)
                        result_percentage = String.format("%.2f", percentage);
                    else
                        result_percentage="0";
                    arrayList.add(new AttendanceDataModel(c.getString(subjectIndex),result_percentage+"%",Integer.toString(c.getInt(totalIndex)),Integer.toString(c.getInt(presentIndex)),Integer.toString(c.getInt(absentIndex))));
